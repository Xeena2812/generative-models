    ...
    def forward(self, x):
        if self.with_conv:
            ...
            x = torch.nn.functional.avg_pool3d(
                x, kernel_size=(2, 2, 2), stride=(2, 2, 2)
            )
        return x
    ...
    def __init__(
        ...
        **ignore_kwargs,
        ...
        self.conv_in = nn.Sequential(
            torch.nn.Conv3d(
                in_channels, self.ch, kernel_size=(3, 3, 3), stride=1, padding=(1, 1, 1)
            ),
        ...
        self.temporal_attn_in = ResidualTemplate(
            in_channels, temporal_attn(in_channels)
        )
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                if curr_res in attn_resolutions:
                    attn.append(
                        ResidualTemplate(block_in, SpatialLinearAttention(block_in))
                    )
        ...
        spatial_attn = EinopsToAndFrom(
            "b c f h w", "b f (h w) c", Attention(block_in, heads=attn_heads)
        )
    ...
    def __init__(
        ...
        **ignorekwargs,
        ...
        self.z_shape = (
            1,
            z_channels,
            *(self.resolution // 2 ** (self.num_resolutions - 1)),
        )
        print(
        ...
        self.conv_in = torch.nn.Conv3d(
            z_channels, block_in, kernel_size=(3, 3, 3), stride=1, padding=(1, 1, 1)
        )
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != 0:
                h = self.up[i_level].upsample(
                    h, tuple(self.resolution // 2 ** (i_level - 1))
                )
